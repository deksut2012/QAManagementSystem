...
namespace Promaxx2.Automation.Pages.Pos;

/// <summary>
/// Base ของ PageObjects สำหรับ PromaxxsPos.exe (บิลขาย)
/// Selector ต้องอ้าง AutomationId เท่านั้น (ตาม SELECTOR_CONTRACT.md)
/// </summary>
public abstract class PosPageBase : IDisposable
{
    protected readonly UIA3Automation Automation = new();
    protected Application App { get; }

    public PosPageBase(Application app) => App = app;

    public Window MainWindow => App.GetMainWindow(Automation)
        ?? throw new InvalidOperationException("PromaxxsPos main window was not found.");

...
public sealed class ModulesController(ProjectService service,ProjectScopeGuard scope):ControllerBase
{
    [HttpPut("{id:guid}")] public async Task<ActionResult<ModuleDto>> Update(Guid id,UpdateModuleRequest request,CancellationToken ct){if(!await scope.ModuleAsync(id,ct))return NotFound();try{return Ok(await service.UpdateModuleAsync(id,request,UserId(),ct));}catch(EntityNotFoundException){return NotFound();}catch(Exception ex)when(ex is ArgumentException or InvalidOperationException){return BadRequest(new ProblemDetails{Title="ข้อมูลไม่ถูกต้อง",Detail=ex.Message,Status=400});}}
    [HttpDelete("{id:guid}")] public async Task<IActionResult> Delete(Guid id,CancellationToken ct){if(!await scope.ModuleAsync(id,ct))return NotFound();try{await service.DeactivateModuleAsync(id,UserId(),ct);return NoContent();}catch(EntityNotFoundException){return NotFound();}catch(InvalidOperationException ex){return BadRequest(new ProblemDetails{Title="ไม่สามารถปิดใช้งาน Module",Detail=ex.Message,Status=400});}}
    [HttpPost("{id:guid}/move")] public async Task<ActionResult<ModuleDto>> Move(Guid id,MoveModuleRequest request,CancellationToken ct){if(!await scope.ModuleAsync(id,ct))return NotFound();try{return Ok(await service.MoveModuleAsync(id,request,UserId(),ct));}catch(EntityNotFoundException){return NotFound();}catch(InvalidOperationException ex){return BadRequest(new ProblemDetails{Title="ไม่สามารถย้าย Module ได้",Detail=ex.Message,Status=400});}}

...
public sealed class DefectsController(QaDbContext db,ProjectAccessContext projectCtx,DefectActivityService activityService,CrmSendToCrmService crmSendService,ILogger<DefectsController> logger):ControllerBase
{
 ...
 [HttpPost("bulk"),Authorize(Policy="DefectEdit")]public async Task<IActionResult>Bulk(BulkDefectRequest r,CancellationToken ct){if(r.Ids is null||r.Ids.Count==0)return BadRequest("No defect IDs provided.");var entities=(await db.Defects.Where(x=>r.Ids.Contains(x.DefectId)&&!x.IsDeleted).ToListAsync(ct)).Where(e=>CanAccess(e.ProjectId)).ToList();var userId=UserId();var isDelete=r.Delete==true;foreach(var e in entities){if(isDelete){e.SoftDelete(userId);}else{if(!string.IsNullOrWhiteSpace(r.Status))e.UpdateStatus(r.Status,userId);if(!string.IsNullOrWhiteSpace(r.Severity))e.UpdateSeverity(r.Severity,userId);if(r.AssigneeUserId.HasValue)e.Assign(r.AssigneeUserId.Value,userId);}}await db.SaveChangesAsync(ct);var action=isDelete?"Deleted":"Updated";var details=isDelete?"Defect deleted by bulk action":$"Defect updated by bulk action{(r.Status is null?string.Empty:$"; Status={r.Status}")}{(r.Severity is null?string.Empty:$"; Severity={r.Severity}")}{(r.AssigneeUserId.HasValue?"; Assignee changed":string.Empty)}";foreach(var e in entities)await activityService.LogAsync(e.DefectId,action,details,userId,ct);return Ok(new{updated=entities.Count});}
...
public sealed record DefectStatsDto(int Total,int Open,int InProgress,int Resolved,int Closed,int Rejected,int Critical,int High,int Medium,int Low,int OldestOpenAgeDays,int UnassignedActive,double ClosureRate);

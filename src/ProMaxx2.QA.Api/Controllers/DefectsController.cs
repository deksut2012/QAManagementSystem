...
[ApiController,Route("api/v1/defects"),Authorize(Policy="DefectEdit")]
public sealed class DefectsController(QaDbContext db):ControllerBase
{
 [HttpGet]public async Task<IReadOnlyList<DefectDto>>List([FromQuery]Guid?projectId,[FromQuery]Guid?releaseId,[FromQuery]Guid?buildId,[FromQuery]string?search,CancellationToken ct){var q=db.Defects.AsNoTracking().Where(x=>!x.IsDeleted);if(projectId.HasValue)q=q.Where(x=>x.ProjectId==projectId);if(releaseId.HasValue)q=q.Where(x=>x.ReleaseId==releaseId);if(buildId.HasValue)q=q.Where(x=>x.BuildId==buildId);if(!string.IsNullOrWhiteSpace(search))q=q.Where(x=>x.DefectCode.Contains(search)||x.Title.Contains(search));return await q.OrderByDescending(x=>x.CreatedAt).Select(x=>new DefectDto(x.DefectId,x.ProjectId,x.ReleaseId,x.BuildId,x.ModuleId,x.DefectCode,x.Title,x.Severity,x.Status,x.CreatedAt)).ToListAsync(ct);}

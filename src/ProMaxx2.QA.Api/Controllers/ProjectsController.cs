...
public sealed class ProjectsController(ProjectService service,ProjectScopeGuard scope):ControllerBase
{
    ...
    [HttpGet("{id:guid}")] public async Task<ActionResult<ProjectDto>> Get(Guid id,CancellationToken ct){if(!scope.Allows(id))return NotFound();try{return Ok(await service.GetAsync(id,ct));}catch(EntityNotFoundException){return NotFound();}}
    ...
    [HttpPut("{id:guid}"),Authorize(Policy="ProjectEdit")] public async Task<ActionResult<ProjectDto>> Update(Guid id,UpdateProjectRequest request,CancellationToken ct){if(!scope.Allows(id))return NotFound();try{return Ok(await service.UpdateAsync(id,request,UserId(),ct));}catch(EntityNotFoundException){return NotFound();}catch(ArgumentException ex){return BadRequest(new ProblemDetails{Title="ข้อมูลไม่ถูกต้อง",Detail=ex.Message,Status=400});}}
    [HttpDelete("{id:guid}"),Authorize(Policy="ProjectEdit")] public async Task<IActionResult> Delete(Guid id,CancellationToken ct){if(!scope.Allows(id))return NotFound();try{await service.DeactivateAsync(id,UserId(),ct);return NoContent();}catch(EntityNotFoundException){return NotFound();}}

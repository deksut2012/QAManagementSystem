...
public sealed class ProductModule
{
    ...
    public Guid ModuleId {get;private set;} public Guid ProjectId {get;private set;} public Guid? ParentModuleId {get;private set;} public string ModuleCode {get;private set;}=string.Empty; public string ModuleName {get;private set;}=string.Empty; public string? Description {get;private set;} public Guid? OwnerUserId {get;private set;} public bool IsActive {get;private set;} public int SortOrder {get;private set;} public DateTime CreatedAt {get;private set;} public Guid? CreatedBy {get;private set;} public DateTime? UpdatedAt {get;private set;} public Guid? UpdatedBy {get;private set;} public Project Project {get;private set;}=null!; public ProductModule? ParentModule {get;private set;}
    ...
    public void Move(Guid? parentModuleId,int sortOrder,Guid? updatedBy){if(parentModuleId==ModuleId)throw new InvalidOperationException("A module cannot be its own parent.");ParentModuleId=parentModuleId;SortOrder=Math.Max(0,sortOrder);UpdatedBy=updatedBy;UpdatedAt=DateTime.UtcNow;}
}

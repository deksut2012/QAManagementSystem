...
public sealed record ReleaseGateInput(int OpenP0, int OpenP1Blockers, bool HasApprovedRisk, bool SmokePassed);

public static class ReleaseGate
{
    public static ReleaseDecision Evaluate(ReleaseGateInput input)
    {
        if (input.OpenP0 > 0 || !input.SmokePassed) return ReleaseDecision.NoGo;

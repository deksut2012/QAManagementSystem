...
public sealed record DashboardModuleHealth(Guid ModuleId,Guid? ParentModuleId,string ModuleName,int Requirements,int CoveredRequirements,int TestCases,int Executed,int Passed,int Failed,int Blocked,decimal CoveragePercent,decimal ExecutionPercent,decimal PassRate,string Health);
...
public sealed record DashboardSummary(int TotalRequirements,int CoveredRequirements,decimal RequirementCoverage,int TotalCases,int ExecutedCases,decimal ExecutionProgress,int PassedCases,decimal PassRate,int OpenP0,int OpenP1,decimal? OverallScore,int TotalDefects,int OpenDefects,int CriticalDefects,int HighDefects,decimal DefectQuality,string RecommendedDecision,IReadOnlyList<DashboardModuleHealth>Modules,IReadOnlyList<DashboardUserPerformance>Users,IReadOnlyList<DashboardStatusSlice>StatusDistribution,DateTime GeneratedAt);

...
public sealed class DashboardRepository(QaDbContext db):IDashboardRepository
{
 public async Task<DashboardSummary>GetAsync(Guid?projectId,Guid?releaseId,Guid?buildId,CancellationToken ct)
 {
  ...
  var defects=await db.Defects.AsNoTracking().Where(x=>!x.IsDeleted&&(!projectId.HasValue||x.ProjectId==projectId)&&(!releaseId.HasValue||x.ReleaseId==releaseId)&&(!buildId.HasValue||x.BuildId==buildId)).Select(x=>new{x.Severity,x.Status}).ToListAsync(ct);

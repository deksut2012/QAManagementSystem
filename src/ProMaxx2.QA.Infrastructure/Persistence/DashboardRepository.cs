...
public sealed class DashboardRepository(QaDbContext db):IDashboardRepository
{
 ...
 {
  var modules=await db.Modules.AsNoTracking().Where(x=>x.IsActive&&(!projectId.HasValue||x.ProjectId==projectId)).Select(x=>new{x.ModuleId,x.ParentModuleId,x.ModuleName}).ToListAsync(ct);
  ...
  var openP0=cycleCases.Count(x=>(x.CurrentStatus=="Fail"||x.CurrentStatus=="Blocked")&&x.Priority=="P0");var openP1=cycleCases.Count(x=>(x.CurrentStatus=="Fail"||x.CurrentStatus=="Blocked")&&x.Priority=="P1");var coverage=Pct(coveredSet.Count,requirements.Count);var progress=Pct(executed,cycleCases.Count);var passRate=Pct(passed,executed);decimal? overall=requirements.Count>0&&cycleCases.Count>0&&executed>0?Math.Round(coverage*.3m+progress*.3m+passRate*.4m,1):null;var decision=requirements.Count==0&&cycleCases.Count==0?"NO DATA":openP0>0?"NO-GO":openP1>0||coverage<90||passRate<90?"CONDITIONAL GO":"GO";
  return new(requirements.Count,coveredSet.Count,coverage,cycleCases.Count,executed,progress,passed,passRate,openP0,openP1,overall,decision,moduleRows,users,[new("Pass",passed,"#16a36a"),new("Fail",failed,"#e5484d"),new("Blocked",blocked,"#f59e0b"),new("Not Run",cycleCases.Count-executed,"#94a3b8")],DateTime.UtcNow);
 }
}

...
public static class DatabaseInitializer
{
    ...
    public static async Task InitializeDatabaseAsync(this IServiceProvider services, string? adminPassword, CancellationToken cancellationToken = default)
    {
        ...
        var assigned=await db.RolePermissions.Where(x=>x.RoleId==adminRole.RoleId).Select(x=>x.PermissionId).ToListAsync(cancellationToken);
        var missing=await db.Permissions.Where(x=>!assigned.Contains(x.PermissionId)).Select(x=>x.PermissionId).ToListAsync(cancellationToken);
        db.RolePermissions.AddRange(missing.Select(x=>new RolePermission(adminRole.RoleId,x)));
